...
export default function ExamStatusPage() {
  ...
  const [examStatus, setExamStatus] = useState<ExamStatus>({
    isEligible: true,
    examDate: '15/06/2026',
    senseiName: 'Levi Felix',
    deadlineDate: '10/06/2026',
    registrationStatus: 'NENHUM'
  });
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // Simulation logic: Pedro Santos is ineligible
      if (parsedUser.name === 'Pedro Santos') {
        setExamStatus(prev => ({ ...prev, isEligible: false }));
      }
    } else {
      setLocation('/');
    }
  ...
  const getRemainingDays = (deadlineStr: string) => {
    // deadlineStr is in DD/MM/YYYY format
  ...
  const nextBelt = user ? getNextBelt(user.currentBelt) : '';
  const remainingDays = getRemainingDays(examStatus.deadlineDate);
        ...
        <div className="bg-secondary/30 border border-white/10 rounded-3xl overflow-hidden">
          <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/5">
            <div className="flex flex-col gap-1">
              ...
              <span className="font-bold text-xl">{user.currentBelt}</span>
            </div>
            <Award className="text-primary/50" size={32} />
          </div>

          <div className="p-6 space-y-8">
            {examStatus.registrationStatus === 'NENHUM' ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <p className="text-xl font-medium leading-relaxed">
                          Você está <span className="text-green-500 font-black uppercase tracking-tight">APTO</span> para fazer o exame para a Faixa <span className="text-white font-bold">{nextBelt}</span> no dia <span className="text-white font-bold">{examStatus.examDate}</span>.
                        </p>
                        <p className="text-white/40 text-sm">Avaliação realizada pelo Sensei <span className="text-white font-medium">{examStatus.senseiName}</span></p>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <p className="text-xl font-medium leading-relaxed">
                          Você está <span className="text-primary font-black uppercase tracking-tight">INAPTO</span> para fazer o exame no dia <span className="text-white font-bold">{examStatus.examDate}</span>.
                        </p>
                        <p className="text-white/40 text-sm">Avaliação realizada pelo Sensei <span className="text-white font-medium">{examStatus.senseiName}</span></p>
                      </div>
                    </div>
                ...
                {examStatus.isEligible && (
                  <div className="flex items-center gap-4 p-4 bg-primary/5 border border-primary/10 rounded-2xl">
                    ...
                    <p className="text-primary text-sm font-semibold">
                      Atenção: Inscrições abertas até o dia <span className="underline decoration-2 underline-offset-4">{examStatus.deadlineDate}</span> (você tem mais <span className="font-bold">{remainingDays} dias</span> para concluir sua inscrição!)
                    </p>
                  </div>
                )}
              ...
              <div className="space-y-8">
                <div className="text-center space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-lg">
                      Você está inscrito(a) para o Exame de Faixa do dia <span className="font-bold text-white">{examStatus.examDate}</span>.
                    </p>
                  </div>

                  <div className="space-y-3">
                    <p className="text-white/40 text-xs uppercase font-bold">Status do Pagamento</p>
                    {examStatus.registrationStatus === 'PENDENTE' ? (
